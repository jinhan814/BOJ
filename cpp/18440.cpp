...
vector<int> _LCS(int l1, int r1, int l2, int r2, bool IsRev = 0) {
	bitset<sz> D, x, S[26];
	if (!IsRev) {
		for (int i = l2; i <= r2; i++) S[b[i] - 'A'][i - l2] = 1;
		for (int i = l1; i <= r1; i++) {
			x = S[a[i] - 'A'] | D;
			D <<= 1, D[0] = 1;
			D = x & (x ^ (x - D));
		...
		for (int i = l2; i <= r2; i++) ret[i - l2 + 1] = ret[i - l2] + D[i - l2];
		return ret;
	}
	else {
		for (int i = r2; i >= l2; i--) S[b[i] - 'A'][r2 - i] = 1;
		for (int i = r1; i >= l1; i--) {
			x = S[a[i] - 'A'] | D;
			D <<= 1, D[0] = 1;
			D = x & (x ^ (x - D));
		...
		vector<int> ret(r2 - l2 + 3);
		for (int i = r2; i >= l2; i--) ret[i - l2 + 1] = ret[i - l2 + 2] + D[r2 - i];
		return ret;
	}
}
...
vector<int> LCS(int l1, int r1, int l2, int r2, bool IsRev = 0) {
	...
	if (sz <= 64) return _LCS<64>(l1, r1, l2, r2, IsRev);
	if (sz <= 128) return _LCS<128>(l1, r1, l2, r2, IsRev);
	if (sz <= 256) return _LCS<256>(l1, r1, l2, r2, IsRev);
	if (sz <= 512) return _LCS<512>(l1, r1, l2, r2, IsRev);
	if (sz <= 1024) return _LCS<1024>(l1, r1, l2, r2, IsRev);
	if (sz <= 2048) return _LCS<2048>(l1, r1, l2, r2, IsRev);
	if (sz <= 4096) return _LCS<4096>(l1, r1, l2, r2, IsRev);
	if (sz <= 8192) return _LCS<8192>(l1, r1, l2, r2, IsRev);
	if (sz <= 16384) return _LCS<16384>(l1, r1, l2, r2, IsRev);
	if (sz <= 32767) return _LCS<32767>(l1, r1, l2, r2, IsRev);
	return _LCS<65536>(l1, r1, l2, r2, IsRev);
}

void Sol(int l1, int r1, int l2, int r2) { // := LCS(a[l1:r1], b[l2:r2])
	// base case
	if (l1 > r1) return;
	if (l1 == r1) {
